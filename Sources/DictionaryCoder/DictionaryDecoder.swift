...
extension DictionaryDecoderImpl: Decoder {
    ...
    where Key: CodingKey {
        ...
        if let value {
            throw DecodingError.typeMismatch(
                [String: DictionaryValue].self,
                .init(
                    codingPath: codingPath,
                    debugDescription:
                        "Expected to decode \([String: DictionaryValue].self) but found \(value.type) instead."
                ))
        } else {
    ...
    private func createError(type: Any.Type, for additionalKey: CodingKey? = nil, value: DictionaryValue?)
        -> DecodingError
    {
        var path = codingPath
...
extension DictionaryDecoderImpl {
    struct KeyedContainer<K: CodingKey>: KeyedDecodingContainerProtocol {
        ...
        func contains(_ key: K) -> Bool {
            if dictionary[key.stringValue] != nil {
                return true
            }
            return false
        }
        ...
        func nestedContainer<NestedKey>(keyedBy type: NestedKey.Type, forKey key: K) throws -> KeyedDecodingContainer<
            NestedKey
